...
const correctFractionalInput = (inval) => {
  let str_val = inval;
  if (typeof inval === 'string') {
    if (inval.indexOf('%2')> 0) {
      str_val = inval.replace(/%2F/i,'/');
    }
    if (inval.indexOf(',')> 0) {
      str_val = inval.replace(/,/,'/');
    }
  }
  return str_val;
}
...
module.exports = {isNumeric, isFractionString, isRadixString, correctFractionalInput, toRational, toRadix, toDecimal, toLSD, fromLSD };